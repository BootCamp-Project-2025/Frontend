...
export const ExperienceSectionForm = ({jobPosition = "", employer= "", country =  "", description= "", startDate= "", endDate= "" , addExperienceCard, closeExperienceForm}) => {

   const { register, formState: { errors, isSubmitting }, handleSubmit, setError, setValue, watch } = useForm({
        defaultValues: {
            jobPosition,
            employer,
            country,
  ...
  return (
    <form onSubmit={handleSubmit((data) => { addExperienceCard({...data, id: crypto.randomUUID()}); closeExperienceForm();  })} className="flex flex-col gap-2 items-start">
      <h1 className="text-center w-full text-2xl font-semibold text-gray-700">Experience Form</h1>
      <TextInput
      ...
      <div className="flex flex-row justify-end w-full mt-2">
        <button type="submit" className="px-7 py-1.5 bg-[#3B82F6] rounded-full cursor-pointer  text-white">Completed</button>
      </div>
